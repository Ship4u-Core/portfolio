...
export function HeroScene({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    // Disarm the head script's hydration deadline. If it already fired the
    // hero is visible in its final state and the load choreography is
    // forfeited; only the scroll choreography runs.
    (window as Window & { __ship4uHydrated?: () => void }).__ship4uHydrated?.();
    const late = document.documentElement.classList.contains("late");
    ...
    const ctx = gsap.context(() => {
      ...
      const counterWrap = root.querySelector<HTMLElement>(
        "[data-hero-counter]",
      );
      const counterBar = root.querySelector<HTMLElement>("[data-counter-bar]");
      const eyebrowTarget = root.querySelector<HTMLElement>(
        "[data-rewrite-target]",
      );
      ...
      /* ---------- Load ---------- */
      if (late) {
        // Hydration missed the deadline: the hero is already complete and
        // the counter reads its final value. Only the wordmark accent is due.
        four?.setAttribute("data-on", "true");
        if (counterWrap) gsap.set(counterWrap, { opacity: 0.6 });
      } else {
        ...
        if (eyebrowTarget) {
          ...
          load.to(
            state,
            {
              ...
              ease: "none",
              onUpdate: () =>
                (ghost.textContent = rewriteFrame(
                  final,
                  state.p,
                  0.3 + state.p,
                )),
        ...
        if (lineMasks.length) {
          ...
          lineMasks.forEach((mask, i) => {
            load.to(
              mask,
              { clipPath: CLIP_FULL, yPercent: 0, duration: 0.9 },
              0.35 + i * 0.06,
            );
          });
        }
          ...
          load
            .to(
              value,
              {
                ...
                ease: "none",
                onUpdate: () =>
                  (counter.textContent = String(Math.round(value.n)).padStart(
                    3,
                    "0",
                  )),
              },
        ...
        if (wipeTargets.length) {
          ...
          load.to(
            wipeTargets,
            { clipPath: CLIP_FULL, duration: 0.8, stagger: 0.06 },
            0.9,
          );
        }
      ...
      lines.forEach((line, i) => {
        scroll.to(
          line,
          { y: () => window.innerHeight * rates[i], ease: "none", duration: 1 },
          0,
        );
      });
      if (rule) {
